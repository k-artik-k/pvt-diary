...
export default function TagManager() {
  ...
  async function handleSave() {
    ...
    setName(''); setPillColor('#3b82f6'); setTextColor('#ffffff'); setEditingId(null); setHexInput('');
    fetchTags();
  }
  ...
  function startEdit(tag) {
    setEditingId(tag.id); setName(tag.name); setPillColor(tag.pill_color); setTextColor(tag.text_color);
  }
  ...
  return (
    <div style={{ padding: '8px 16px' }}>
      <div style={{ marginBottom: 12 }}>
        <input
          type="text" placeholder="Tag name (max 20)" value={name}
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 8 }}>
          {DEFAULT_COLORS.map(c => (
            <button key={c} onClick={() => setPillColor(c)}
              style={{ width: 20, height: 20, borderRadius: '50%', background: c, border: pillColor === c ? '2px solid var(--text)' : '2px solid transparent', cursor: 'pointer' }}
            />
          ))}
        </div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
          <input type="text" placeholder="#hex" value={hexInput} onChange={e => { setHexInput(e.target.value); if (/^#[0-9a-fA-F]{6}$/.test(e.target.value)) setPillColor(e.target.value); }}
        ...
        {tags.map(tag => (
          <div key={tag.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 0' }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              ...
              <button onClick={() => handleDelete(tag.id)} style={{ fontSize: 12, color: 'var(--danger)', cursor: 'pointer' }}>×</button>
            </div>
          </div>
        ))}
